...
# Define the metrics to plot matching the SPSS analysis
metrics = {
    'Speed_ms': {
...
for col, info in metrics.items():
    ...
    # Draw bars with standard SPSS blue color (#008fd5 or #1f77b4) and error bars
    # Custom color matches the SPSS sky blue in the user's reference image
    spss_blue = '#008fd5'
    ...
    # X-axis label matching SPSS formatting
    ax.set_xlabel('Platform', fontsize=11, labelpad=12)
    
    # Set y-axis limits to start at 0 with 20% breathing room at the top
    max_value = (stats['mean'] + stats['ci95']).max()
    ax.set_ylim(0, max_value * 1.2)
    
    # Draw horizontal gridlines (SPSS default gridlines are very light gray)
